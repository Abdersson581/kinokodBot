...
(function initTrailerFsButton() {
  ...
  fsBtn.addEventListener('click', (e) => {
    ...
    if (typeof root.requestFullscreen === 'function' || typeof root.webkitRequestFullscreen === 'function') {
      ...
      _requestFs(frame && !frame.classList.contains('hidden') ? frame : (modal || undefined));
      return;
    }
    // 3) iOS: браузерного fullscreen нет. Наш mp4 умеет своё
    //    webkitEnterFullscreen; для YouTube-iframe — фолбэк в YouTube.
  ...
  try {
    if (tg && typeof tg.onEvent === 'function') {
      ...
      tg.onEvent('fullscreenFailed', (e) => {
        // UNSUPPORTED — клиент не умеет полноэкранный режим мини-аппа (Telegram
        // Web, Desktop или старый клиент). Двух таких ответов достаточно, чтобы
        // больше не дёргать метод и сразу падать на фолбэки.
